...
func GetRecords(cond models.IRecord) ([]models.IRecord, error) {
	fmt.Println("in getRecords")
	fmt.Print(cond.GetType())
	return (recordsDAO{}).GetAll(database.Client,
		cond,
	)
}

func GetRecordsMF(domain string) ([]models.Record[models.RecordContentDefault], error) {
	r, err := (recordsDAO{}).GetAll(database.Client, &models.Record[models.RecordContentDefault]{Zone: domain}, &models.Record[models.RecordContentDefault]{Zone: domain})
	n := make([]models.Record[models.RecordContentDefault], 0)
	for _, e := range r {
		i, ok := e.(*models.Record[models.RecordContentDefault])
		if !ok {
			continue
		}
		n = append(n, *i)
